...
class RecaptchaVerifier:
    ...
    def create_assessment(self, token: str, recaptcha_action: str) -> Assessment:
        """
    ...
    def verify_token(
        self, token: str, recaptcha_action: str, min_score: float = 0.5
    ) -> dict:
        """
        ...
        try:
            ...
            if score < min_score:
                return {
                    ...
                    "min_score": min_score,
                    "reasons": [
                        reason.name for reason in assessment.risk_analysis.reasons
                    ],
                }
            ...
            return {"success": False, "error": f"Error en verificación: {str(e)}"}
...
def setup_recaptcha_routes(server):
    ...
    def verify_recaptcha():
        ...
        result = recaptcha.verify_token(
            token=token, recaptcha_action=action, min_score=0.5
        )
